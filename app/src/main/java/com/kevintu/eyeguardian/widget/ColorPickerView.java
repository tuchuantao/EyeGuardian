package com.kevintu.eyeguardian.widget;
...
public class ColorPickerView extends View {
  ...
  private void initView() {
    ...
    mCenterPaint.setColor(Color.parseColor("#55FF0000"));
  ...
  protected void onDraw(Canvas canvas) {
    //移动中心
    canvas.translate(getMeasuredWidth() / 2, getMeasuredHeight() / 2 - 50);
    //画中心圆
    canvas.drawCircle(0, 0, centerRadius, mCenterPaint);
    //是否显示中心圆外的小圆环
    if (highlightCenter || highlightCenterLittle) {
      paintColor = mCenterPaint.getColor();
      ...
      mCenterPaint.setColor(paintColor);
    }
    //画色环
    ...
    offset = mLinePaint.getStrokeWidth() / 2;
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    float y = event.getY() - getMeasuredHeight() / 2 + 50;
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_MOVE:
        if (downInCircle && inCircle) {//down按在渐变色环内, 且move也在渐变色环内
          ...
          mCenterPaint.setAlpha(100);
          Log.v(ColorPickerView.class.getSimpleName(), "色环内, 坐标: " + x + "," + y);
        } else if (downInRect && inRect) {//down在渐变方块内, 且move也在渐变方块内
          mCenterPaint.setColor(interpRectColor(mRectColors, x));
        }
        Log.v(ColorPickerView.class.getSimpleName(), "[MOVE] 高亮: " + highlightCenter + "微亮: " + highlightCenterLittle + " 中心: " + inCenter);
